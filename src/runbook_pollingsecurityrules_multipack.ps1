...
[Parameter(Mandatory=$false)][string]$querypackname="sentinel-like-security-queries,mysql-security-queries,sql-security-queries",
...
# Support multiple query packs (comma-separated)
$securityrules = @()
foreach ($pack in $querypackname.Split(",").Trim()) {
    "Loading rules from query pack: $pack"
    $packrules = get-savedqueries -environment $cloud -subscriptionId $subscriptionId -resourceGroup $resourcegroupname -querypackname $pack -registeredruletype $registeredruletype
    if ($null -ne $packrules) { $securityrules += $packrules }
}
"Total rules loaded: $($securityrules.Count)"
...
$srule = get-savedqueries -environment $cloud -subscriptionId $subscriptionId -resourceGroup $resourcegroupname -querypackname $querypackname.Split(",")[0].Trim() -rulename $srulename
...
# disable least common processes execution as it it noisy
        ...
        if (($securityrule.properties.displayname -notin $noisyrules) -and $securityrule.properties.properties.enabled) {
            ...
            if ($NLLL -ne $result) {
                    ...
                    if ($securityrule.properties.properties.Category -in ('HIDS','DSM')) {
                    ...
                    } elseif($securityrule.properties.properties.Category -eq 'AzureSecurityCenter') {
                        ...
                        foreach ($resultobj in $result) {
                            ...
                            $queryresult += [PSCustomObject]@{
                                    Category = $securityrule.properties.properties.Category
                                    rulename = $resultobj.AlertName
                                    description = $resultobj.Description
                                    type = $securityrule.properties.properties.type
                                    query = $query
                                    severity =  $resultobj.AlertSeverity
                                    count = 1
                                    timespan = $timespan
                                    pid = $PIDstr
                                    url = $alerturl
